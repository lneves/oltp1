...
# TPC-E EGenLoader Parameter Guide (v1.14.0)

This guide provides a detailed explanation of the command-line options for the official TPC-E `EGenLoader` tool. Understanding these parameters is essential for correctly generating a compliant and scalable TPC-E database.
...
-----
  ...
  * **`-i <dir>` (Input Directory)**
      ...
      * **Default**: `flat_in/`
      * Specifies the directory where `EGenLoader` will look for its static input files (e.g., `StatusType.txt`, `TaxRates.txt`, `LastName.txt`).
  ...
  * **`-l [FLAT|ODBC|CUSTOM|NULL]` (Load Type)**

      * **Default**: `FLAT`
      * Defines the output method:
          * **`FLAT`**: Generates data as text files (`.txt`) in the output directory, which can then be loaded into the database using native bulk-loading tools.
          * **`ODBC`**: Loads data directly into the database via an ODBC connection.
          * **`CUSTOM`**: For custom, user-defined loader libraries.
          * **`NULL`**: Performs the full data generation process but discards the output. This is useful for performance testing the `EGen` tool itself.

  * **`-m [APPEND|OVERWRITE]` (Output Mode)**

      * **Default**: `OVERWRITE`
      * When using the `FLAT` load type, this controls whether existing files in the output directory are overwritten or appended to.
