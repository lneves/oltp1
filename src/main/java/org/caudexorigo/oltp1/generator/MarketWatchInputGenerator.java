...
import org.caudexorigo.oltp1.model.RandomCustomer;
...
public class MarketWatchInputGenerator
{
	...
	private static final int PERCENT_BY_INDUSTRY = 5;
	private static final int PERCENT_BY_WATCH_LIST = 60;
	...
	public TxMarketWatchInput generateMarketWatchInput()
	{
		...
		if (threshold <= PERCENT_BY_INDUSTRY)
		{
		...
		else if (threshold <= PERCENT_BY_INDUSTRY + PERCENT_BY_WATCH_LIST)
		{
	...
	private final LocalDate getStartDay(CRandom random)
	{
		...
		// + 5 ensures the selected date is always at least five weeks into the historical data.
        ...
        // to simulate realistic user behavior where queries favor the start and end of the week.
		...
		LocalDate startDate = DAILY_MARKET_BASE_DATE.plusDays(totalDayOffset);
