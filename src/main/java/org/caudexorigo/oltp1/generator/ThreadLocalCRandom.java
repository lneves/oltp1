...
/**
 * A thread-safe wrapper for the CRandom class using ThreadLocal. 
 * Each thread gets its own instance of CRandom, initialized with a unique seed.
 */
public final class ThreadLocalCRandom
{
	...
	private static final ThreadLocal<CRandom> threadLocalRandom = new ThreadLocal<>()
	{
		...
		protected CRandom initialValue()
		{
			...
			// getAndIncrement() is an atomic operation, ensuring no two threads get the same seed.
