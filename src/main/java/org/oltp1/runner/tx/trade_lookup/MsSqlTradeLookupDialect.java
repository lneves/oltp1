...
/**
 * Provides the Microsoft SQL Server-specific SQL queries for the Trade-Lookup
 * transaction.
 */
public class MsSqlTradeLookupDialect extends DefaultTradeLookupDialect
{
	...
	public String buildTradeIdList(Collection<Long> tradeIds)
	{
		if (tradeIds.isEmpty())
		{
			// This prevents an empty list and ensures `trade_ids` is never null or blank,
			// per Microsoft documentation, an empty input yields a single empty substring,
			// and CAST('' AS BIGINT) raises a conversion error. TxTradeLookup catches the
			// exception a and records status -1, replacing the intended warnings -611/631.
			return "0";
		}
