...
/**
 * Provides the Microsoft SQL Server-specific SQL queries for the Trade-Update
 * transaction.
 */
public class MsSqlTradeUpdateQueries extends DefaultTradeUpdateQueries
{
	...
	public String getTradeInfo()
	{
		return """
				...
				WHERE t.t_id IN (SELECT CAST(value AS bigint) AS trade_id FROM STRING_SPLIT(:trade_lst,','));
				""";
	}
	...
	public String getSettlementInfo()
	{
		return """
				...
				FROM settlement
				WHERE se_t_id IN (SELECT CAST(value AS bigint) AS trade_id FROM STRING_SPLIT(:trade_lst,','))
				""";
	}
	...
	public String getTradeHistory()
	{
		return """
				...
				FROM trade_history
				WHERE th_t_id IN (SELECT CAST(value AS bigint) AS trade_id FROM STRING_SPLIT(:trade_lst,','))
				ORDER BY th_dts
				""";
	}
	...
	public String getCashTransactionInfo()
	{
		return """
				...
				WHERE ct_t_id IN (SELECT CAST(value AS bigint) AS trade_id FROM STRING_SPLIT(:trade_lst,','))
				""";
	}
}
